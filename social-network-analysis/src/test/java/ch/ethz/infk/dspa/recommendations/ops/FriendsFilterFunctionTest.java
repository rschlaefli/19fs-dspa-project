...
public class FriendsFilterFunctionTest {
	...
	void setup() throws IOException {
		similarities = new PersonSimilarityTestDataGenerator()
				.generate("src/test/java/resources/recommendations/streams/person_similarity_stream.csv");
	}

	@Test
	void testFriendsFilterFunction_loadsExpectedKnowsRelation() throws Exception {
		FriendsFilterFunction filterFunction = new FriendsFilterFunction(
				"src/test/java/resources/recommendations/relations/person_knows_person.csv");
		...
		assertTrue(friendsWithP1.containsAll(Arrays.asList(2L, 3L, 4L, 7L)),
				"Friends of P1 do not match expected values.");
	...
	void testFriendsFilterFunction_filtersAsExpected() throws Exception {
		...
		for (PersonSimilarity expectedSimilarity : expectedSimilarities) {
			assertTrue(actualSimilarities.remove(expectedSimilarity),
					"Expected similarity " + expectedSimilarity + " not present!");
		}
